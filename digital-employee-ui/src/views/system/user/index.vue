        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="角色">
              <el-select v-model="form.roleIds" multiple placeholder="请选择角色" @change="handleRoleChanged">
                <el-option
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getUser(userId).then(response => {
        ...
        if (response.data.userDeConfig) {
          this.showConfigBox = true;
          this.configForm = response.data.userDeConfig;
        }
        // this.postOptions = response.posts;
        this.roleOptions = response.roles;
    ...
    submitForm: function () {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.showConfigBox) {
            this.form.userDeConfig = {...this.configForm}
          }
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除用户编号为"' + userIds + '"的数据项？ 删除该数据会同时删除用此用户创建的服务对象！').then(function () {
